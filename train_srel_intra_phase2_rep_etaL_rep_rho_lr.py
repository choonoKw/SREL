...
def main(save_weights, save_logs, save_mat, learning_rate, lambda_eta):
    # Load dataset
    ...
    log_dir = (
        f'runs/intra_phase2/rep_etaL/data{data_num:.0e}/{start_time_tag}'
    ...
    if save_mat:
        ...
        dir_mat_save = (
            f'mat/intra_phase2/rep_etaL/{start_time_tag}'
    ...
    if save_weights:
        ...
        # save
        dir_weight_save = (
            f'weights/intra_phase2/rep_etaL/{start_time_tag}'
            f'_Nstep{N_step:02d}_batch{batch_size:02d}'
            f'_sinr_{worst_sinr_avg_db:.2f}dB'
        )
...
if __name__ == "__main__":
    ...
    main(save_weights=args.save_weights, save_logs=args.save_logs,save_mat=args.save_mat, learning_rate=1e-7, lambda_eta=1e-9)
    
    main(save_weights=args.save_weights, save_logs=args.save_logs,save_mat=args.save_mat, learning_rate=1e-7, lambda_eta=1e-7)
    
    main(save_weights=args.save_weights, save_logs=args.save_logs,save_mat=args.save_mat, learning_rate=1e-7, lambda_eta=1e-5)
    
    main(save_weights=args.save_weights, save_logs=args.save_logs,save_mat=args.save_mat, learning_rate=1e-7, lambda_eta=1e-3)
    
    main(save_weights=args.save_weights, save_logs=args.save_logs,save_mat=args.save_mat, learning_rate=1e-7, lambda_eta=1e-1)    
