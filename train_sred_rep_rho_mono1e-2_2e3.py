...
#    model_intra_phase1.apply(init_weights)
    ...
    log_dir = (
        f'runs/sred_rep_rho_mono_2e3/data{data_num:.0e}/'
        f'lambda_sinr{lambda_sinr:.0e}/'
        f'{start_time_tag}'
    ...
    for epoch in range(num_epochs):
        ...
        ## extra learning
        dataset = TrainingDataSet(f'data/data_trd_2e3.mat')
        y_M = dataset.y_M.to(device)  # If y_M is a tensor that requires to be on the GPU
    ...
    if save_mat:
        ...
        dir_mat_save = (
            f'mat/sred_rep_rho_mono_2e3/{start_time_tag}'
    ...
    if save_weights:
        ...
        # save
        dir_weight_save = (
            f'weights/sred_rep_rho_mono_2e3/{start_time_tag}'
            f'_Nstep{N_step:02d}_batch{batch_size:02d}'
            f'_sinr_{worst_sinr_avg_db:.2f}dB'
        )
