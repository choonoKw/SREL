...
class SREL_rep_ss_inter(nn.Module):
    def __init__(self, constants, model_intra):
        super(SREL_rep_ss_inter, self).__init__()
        ...
        self.est_mu_module = Estimate_mu(2*self.Ls + 2*constants['Lw'] + constants['Ly'], 1)
    ...
    def forward(self, phi0_batch, w_M_batch, y_M):
        ...
        mu_M_stack_batch = torch.zeros(batch_size, N_step, M).to(self.device)
        ...
        for idx_batch in range(batch_size):
            ...
            for update_step in range(N_step):
                ...
                for m in range(M):
                    ...
                    mu = self.est_mu_module(x)
                    
                    eta_net += mu*rho*eta
                
                    mu_M_stack_batch[idx_batch,update_step,m] = mu
        ...
        model_outputs = {
            ...
            'mu_M_stack_batch': mu_M_stack_batch
        }
